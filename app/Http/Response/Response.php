<?php
    ...
    // response statusCode codes {{{
        ...
        public function sendHeaders(){
            $header = sprintf('HTTP/1.0 %s %s', $this->statusCode, self::$statusTexts[$this->statusCode]);
        ...
        public function __call($name, $args) {
            if (substr($name, 0, 4) == "with") {
                $value = strtolower(substr($name, 4));
                $this->{$value} = $args[0];
            }
            return $this;
        }

        public function setContent($ret) {

            $this->charset = 'UTF-8';
 
            if ($ret instanceof \App\Views\View) {
                $this->content = $ret->getContent();
            }
